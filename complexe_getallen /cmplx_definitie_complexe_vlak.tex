...
We kunnen elk complex getal $z=a+bi\in \C$ voorstellen als een punt in het reële  vlak, namelijk als het punt
met cartesische coördinaten $(a,b)$ (met $a,b$ dus reële getallen). 
...
We spreken in deze context ook van {\bf het complexe vlak} of {\bf het vlak van Gauss}.
\\
De \textit{zuiver reële} getallen bevinden zich op de (horizontale) $x-$as, die we dan ook de \textbf{reële as} noemen.
\\
De \textit{zuiver imaginaire} getallen bevinden zich op de (vertikale) $y-$as, die we de \textbf{imaginaire as} noemen.
...
\addplot[blue,mark=*] coordinates {(3,2)} node[pin=60:{$3+2i$}]{} ;
\addplot[blue,mark=*] coordinates {(-2,3)} node[pin=90:{\small$(3+2i)\cdot i = -2+3i$}]{} ;

\addplot[red,mark=*] coordinates {(-2,1)} node[pin=150:{$-2+i$}]{} ;
\addplot[red,mark=*] coordinates {(-1,-2)} node[pin=260:{\small$(-2+i)\cdot i = -1-2i$}]{} ;
